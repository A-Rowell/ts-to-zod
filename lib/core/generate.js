 ...
 */
function generate({ sourceText, nameFilter = () => true, jsDocTagFilter = () => true, getSchemaName = (id) => (0, case_1.camel)(id) + "Schema", keepComments = false, skipParseJSDoc = false, customJSDocFormatTypes = {}, }) {
    ...
    const visitor = (node) => {
        if (typescript_1.default.isInterfaceDeclaration(node) ||
            ...
            if (!nameFilter(node.name.text))
                return;
            const typeNames = (0, traverseTypes_1.getExtractedTypeNames)(node, sourceFile);
